...
export function upiRetry(outcomes) {
  ...
  const result = {attempts : 0, success: false, totalWaitTime: 0};
  let backoff = 0;
  
  do{
    result.attempts++;
    if(outcomes[result.attempts-1] === "success"){
      result.success = true;
      break;
    }
    if(result.attempts === 5) break;
    result.totalWaitTime += Math.pow(2,backoff);
    backoff++;
